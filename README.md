<h1 align="center">Hi 👋, I'm Shelly Verma</h1>
<h3 align="center">Backend Developer | Java | Spring Boot | Open Source Contributor</h3>

---

### 👩‍💻 About Me
- 🎓 Final-year CSE student at JSS Academy of Technical Education, Noida  
- 💻 Passionate about Backend & Full-Stack Development  
- 🌱 Currently learning advanced Spring Boot & System Design  
- 🚀 Ex-GSSoC '24 Contributor | GSSoC '26 Applicant  
- 📫 Reach me at: **shellyverma1015@gmail.com**

---

### 🚀 Tech Stack
- 💻 Languages: Java, Python, JavaScript, SQL  
- ⚙️ Backend: Spring Boot, Node.js, Express  
- 🌐 Frontend: React, HTML, CSS  
- 🛢️ Database: MySQL  
- 🛠️ Tools: Git, GitHub, Postman, VS Code  

---

### 🔥 Featured Projects

#### 🍔 Multi-Restaurant Food Ordering System
- Full-stack app with Admin, Customer, Restaurant & Delivery roles  
- JWT Authentication & Role-Based Access  
- Built using Spring Boot, React & MySQL  

#### 🎥 StreamHub – Video Chat App
- Real-time video communication using WebRTC & Socket.io  
- Private rooms & authentication system  
- Built with Node.js & Express  

#### 🧠 Heart Disease Prediction (ML)
- Machine learning model with 92% accuracy  
- Built using Python, Pandas, NumPy  

---

### 🌟 Achievements
- 🏆 GSSoC 2024 Contributor  
- 🔥 Open Source Contributions (PRs, bug fixes, improvements)

---

### 🤝 Connect with Me
- 💼 LinkedIn: https://www.linkedin.com/in/shelly-verma-267718255/
- 💻 GitHub: https://github.com/shellyverma
